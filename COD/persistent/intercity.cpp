...
struct {
  ...
  void solve() {
    ...
    for (i32 i = 0, p = 0; i < q; ++i) {
      ...
      i32 l = a + p, r = b + p;
      i32 ind = [&] -> i32 {
        i32 lq = -1, rq = static_cast<i32>(events.size());
        for (; rq - lq > 1;) {
          i32 mq = (lq + rq) >> 1;
          events[mq].x < r ? lq = mq : rq = mq;
        }
        return lq + 1;
      }();
      cout << (p = st.get(l, vers[ind]) + 1) << '\n';
    }
  }
  void precalculate();
} solution;
