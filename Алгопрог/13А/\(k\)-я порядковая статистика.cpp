...
public:
  pers_st (i32 sz_): sz{sz_} { rt.push_back(0); }
...
struct {
  void solve() {
    ...
    std::sort(all(a_));
    mpi32 match, match2;
    for (i32 i = 0; i < n; ++i) match[a_[i]] = i;
    for (i32 i = 0; i < n; ++i) match2[a[i]] = i;
    pers_st st(n);
    vi32 vers(n + 1);
    for (i32 i = 0; i < n; ++i) {
      vers[i + 1] = st.update(match[a[i]], 1);
    }
    i32 m;
    cin >> m;
    for (i32 i = 0; i < m; ++i) {
      i32 l, r, k;
      cin >> l >> r >> k; --l;
      cout << match2[a_[st.get(vers[l], vers[r], k)]] - l << '\n';
    }
  }
  void precalculate();
} solution;
