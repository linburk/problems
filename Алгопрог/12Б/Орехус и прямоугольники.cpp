...
struct line {
    ...
    i64 operator*(line const &rhs) {
        i64 x = (rhs.b - b) / (k - rhs.k);
        if (b > rhs.b)
            --x;
        return x;
    }
...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        for (auto&& [x, y, a] : p)
            cin >> x >> y >> a;
        std::sort(all(p), [](rect const &lhs, rect const &rhs) {
            return lhs.x > rhs.x;
        });
        vi64 dp(n + 1);
        v<line> lines;
        vi64 dots;
        auto get = [&] (i64 x) {
            i64 idx = std::lower_bound(all(dots), x, std::greater<i64>()) - dots.begin() - 1;
            return lines[idx](x);
        };
        auto add = [&] (line n) {
            for (; lines.size() && lines.back()(dots.back()) < n(dots.back());)
                lines.pop_back(), dots.pop_back();
            if (lines.empty()) {
                dots.push_back(1E9);
            } else {
                dots.push_back(lines.back() * n);
            }
            lines.push_back(n);
        };
        add({0, 0});
