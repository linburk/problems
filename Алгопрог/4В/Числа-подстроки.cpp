    ...
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        for (; q.size();) {
            ...
            for (i32 i = 1; i <= s.size(); ++i) {
                for (i32 j = 0; j + i <= s.size(); ++j) {
                    i32 add = std::stoi(s.substr(j, i));
                    i32 cost = 0;
                    for (i32 t = add; t; t /= 10)
                        cost += t % 10;
                    for (i32 k = -1; k <= 1; k += 2) {
                        if (v + k * add > 0 && v + k * add <= n && dist[v + k * add] > dist[v] + cost) {
                            q.erase({dist[v + k * add], v + k * add});
                            dist[v + k * add] = dist[v] + cost;
                            pr[v + k * add] = k * add;
                            q.insert({dist[v + k * add], v + k * add});
                        }
                    }
                }
            }
