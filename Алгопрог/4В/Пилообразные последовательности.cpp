...
struct {
  ...
  void solve() {
    i32 n, k;
    cin >> n >> k;
    vvi64 dp1(n + 1, vi64(k + 2)), dp2(n + 1, vi64(k + 2));
    // 1 - xX
    // 2 - Xx
    dp1[0][k + 1] = dp2[0][0] = 1;
    for (i32 i = 1; i <= n; ++i) {
      ...
      for (i32 j = 0; j < k + 1; ++j) {
        pref2[j + 1] = (pref2[j] + dp2[i - 1][j]) % mod;
      }
      for (i32 j = k + 1; j >= 1; --j) {
        suff1[j - 1] = (suff1[j] + dp1[i - 1][j]) % mod;
      }
      for (i32 j = 1; j <= k; ++j) {
        dp1[i][j] = (dp1[i][j] + pref2[j]) % mod;
        dp2[i][j] += (dp2[i][j] + suff1[j]) % mod;
      }
    }
    i64 res = 0;
    for (i32 j = 1; j <= k; ++j) {
