...
struct l_ {
  i64 a, b, c;
  l_() {}
  l_(i64 a_, i64 b_, i64 c_): a{a_}, b{b_}, c{c_} {}
  l_(p_ const &fp, p_ const &sp) {
    a = fp.y - sp.y;
    b = sp.x - fp.x;
    c = -(a * sp.x + b * sp.y);
    i64 gcd = std::gcd(a, std::gcd(b, c));
    a /= gcd, b /= gcd, c /= gcd;
  }
  f64 y(i64 x) {
    return (- a * x - c) * 1.L / b;
...
struct {
  void solve() {
    ...
    i32 l_p = 0, r_p = 0;
    for (i32 i = 0; i < n; ++i) {
      if (pnt[i].x < pnt[l_p].x) l_p = i;
      if (pnt[i].x > pnt[r_p].x) r_p = i;
    }
    vector<p_> up, down;
    for (i32 i = l_p; i != r_p; i = (i + 1) % n)
      up.push_back(pnt[i]);
    up.push_back(pnt[r_p]);
    for (i32 i = r_p; i != l_p; i = (i + 1) % n)
      down.push_back(pnt[i]);
    down.push_back(pnt[l_p]);
    std::reverse(all(down));
    bool inf = 0;
    i32 count = 0;
    f64 eps = 1E-9;
    for (i32 l = 1, r = 1;;) {
      for (; r + 1 < down.size() && down[r].x < up[l - 1].x; ++r);
      for (; l + 1 < up.size() && up[l].x < down[r - 1].x; ++l);
      i64 l_x = std::max(down[r - 1].x, up[l - 1].x);
      i64 r_x = std::min(up[l].x, down[r].x);
      l_ up_line(up[l], up[l - 1]);
      l_ down_line(down[r], down[r - 1]);
      f64 l_d = std::abs(up_line.y(l_x) - down_line.y(l_x));
      f64 r_d = std::abs(up_line.y(r_x) - down_line.y(r_x));
      if (l_x != r_x && std::abs(l_d - d) < eps && std::abs(r_d - d) < eps)
        inf = 1;
      if (l_d - eps < d && d < r_d)
        ++count;
      if (l_d + eps > d  && d > r_d)
        ++count;
      if (l + 1 < up.size() && up[l].x <= down[r].x)
        ++l;
      else if (r + 1 < down.size() && down[r].x <= up[l].x)
        ++r;
      else break;
    }
