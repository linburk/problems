...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        vvi32 gr2(n);
        std::function<void(i32)> dfs = [&] (i32 v) {
            static vi32 cur;
            cur.push_back(v);
            cond[v] = 1;
            for (auto [to, idx] : gr[v]) {
                if (used[idx])
                    continue;
                used[idx] = 1;
                ans[idx] = {v, to};
                gr2[v].push_back(to);
                if (cond[to]) {
                    for (; cur.size() && cur.back() != to; cur.pop_back())
                        cond[cur.back()] = 2;
                } else {
                    dfs(to);
                }
            }
            if (cur.size() && cur.back() == v)
                cur.pop_back();
            if (cond[v] != 2) {
                cond[v] = 0;
            }
        ...
        // ÐºÐ°Ðº Ð±ÑÐ´ÑÐ¾ Ð¼Ð¾Ð¶Ð½Ð¾ Ð·Ð° Ð»Ð¸Ð½Ð¸Ñ, Ð½Ð¾ Ñ ÑÐµÑ Ð½Ðµ Ð´Ð¾ÐºÑÑÑÐ¸Ð» Ð¸Ð´ÐµÑ
    }
