...
struct solution {
    ...
    auto operator()(i32 n_test = 0) -> void // solve
    {
...
#endif
        i32 p = 150;
        vvpi32 dp(n + 1, vpi32(n, {1E9, 1E9}));
        vvi32 pow(10, vi32(p));
        for (i32 i = 0; i < 10; ++i) {
            pow[i][0] = 1;
            pow[i][1] = i;
            for (i32 j = 2; j < p; ++j) {
                pow[i][j] = pow[i][j - 1] * 10 % n;
            }
        }
        dp[0][0] = {0, 0};
        std::bitset<1000> was;
            ...
            for (i32 m = 0; m < n; ++m) {
                if (dp[s][m].first != 1E9) {
                    for (i32 k = 1; k < 10; ++k) {
                        i32 ns = s + k;
                        if (s + k <= n) {
                            for (i32 i = dp[s][m].first + 1; i < std::min(p, dp[s][m].first + 10); ++i) {
                                i32 nl = i;
                                i32 nm = (m + pow[k][i]) % n;
                                if (was[nm])
                                    break;
                                ...
                                dp[ns][nm] = std::min(dp[ns][nm], {nl, k});
                            }
                            was.reset();
                        }
                    }
        ...
        for (; s;) {
            ...
            rs += ld + '0';
            i32 ns = s - ld;
            i32 nm = (m - pow[ld][len] + n) % n;
            i32 nlen = dp[ns][nm].first;
            for (i32 j = 0; j < len - 1 - nlen; ++j)
                rs += '0';
            s = ns;
            m = nm;
        }
