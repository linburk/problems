...
struct solution {
    v<string> a;
    i32 mxd = 0;
    void rec(i32 l, i32 r, i32 d) {
        if (d == mxd)
            return;
        i32 del = l - 1;
        vpi32 nx;
        for (i32 i = l; i < r; ) {
            if (i > l && a[i][d] < a[i - 1][d])
                del = i;
            i32 j = i;
            for (; j < r && a[j][d] == a[i][d]; ++j)
                ;
            nx.push_back({i, j});
            i = j;
        }
        for (i32 i = l; i < del; ++i)
            a[i].erase(d, a[i].size());
        for (i32 i = 0; i < nx.size(); ++i) {
            if (nx[i].first >= del) {
                rec(nx[i].first, nx[i].second, d + 1);
            }
        }
    }
