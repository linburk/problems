...
struct trie {
private:
  static const i32 bit_c = 32;
  struct node {
    i32 to[2] = {0, 0};
    i32 term = 0;
  };
  vector<node> nd{node()};
  void find(vi32 &path, i32 x, i32 pos = 0, i32 n = 0) {
    path.push_back(n);
    if (pos == bit_c)
      return;
    if (x & (1 << (bit_c - 1 - pos))) {
      if (!nd[n].to[1]) {
        nd[n].to[1] = static_cast<i32>(nd.size());
        nd.push_back(node());
      }
      find(path, x, pos + 1, nd[n].to[1]);
    } else {
      if (!nd[n].to[0]) {
        nd[n].to[0] = static_cast<i32>(nd.size());
        nd.push_back(node());
      }
      find(path, x, pos + 1, nd[n].to[0]);
    }
  }
public:
  void del(i32 x) {
    vi32 path; find(path, x);
    for (auto i : path)
      --nd[i].term;
  }
  void add(i32 x) {
  ...
  i32 max(i32 x, i32 pos = 0, i32 n = 0, i32 val = 0) {
    if (pos == bit_c)
      return val;
    i32 bit = (x & (1 << (bit_c - 1 - pos))) >> (bit_c - 1 - pos);
    if (nd[n].to[bit ^ 1] && nd[nd[n].to[bit ^ 1]].term)
      return max(x, pos + 1, nd[n].to[bit ^ 1], val ^ (1 << (bit_c - 1 - pos)));
    else
      return max(x, pos + 1, nd[n].to[bit], val);
  }
};
