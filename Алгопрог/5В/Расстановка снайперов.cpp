...
namespace vectors {
    ...
    struct vec {
        ...
        const vec operator+(const vec &C) const { return vec(*this) += (C.B - C.A); }
...
void solve() {
    point A, B, C;
    cin >> A >> B >> C;
    vec AB(A, B), AC(A, C), BC(B, C), BA(B, A), CA(C, A), CB(C, B);
    vec res;
    if (abs(AB * AC) < 1e-6){
        res = AB + AC;
    }
    else if (abs(BC * BA) < 1e-6){
        res = BC + BA;
    }
    else if (abs(CA * CB) < 1e-6){
        res = CA + CB;
    }
    cout << res.B.x << " " << res.B.y;
}
