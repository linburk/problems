        ...
        for (int k = 0; k < adjMatrix->size(); k++){
                for (int i = 0; i < adjMatrix->size(); i++){
                    for (int j = 0; j < adjMatrix->size(); j++){
                        transClosure->at(i)[j] = transClosure->at(i)[j] || transClosure->at(i)[k] && transClosure->at(k)[j];
                    }
                }
            }
    }
        ...
        for (int a = 0; a < transClosure->size(); a++){
            for (int b = 0; b < transClosure->size(); b++){
                for (int c = 0; c < transClosure->size(); c++){
                    if (transClosure->at(a)[b] && transClosure->at(b)[c] && adjMatrix->at(a)[c]){
                        adjMatrix->at(a)[c] = 0;
                    }
                }
            }
...
struct _condensateGraph{
    ...
    operator()(std::vector<std::vector<int> > &graph, std::vector<std::vector<int> > &turnedGraph)
    {
        doTopSort(graph);
...
void getNewGraph(vector<vector<int> > &graph, vector<vector<int> > &turnedGraph){
    ...
    for (int i = 0; i < compMb.size(); i++){
        for (int v = 0; v < compMb[i].size() && compMb[i].size() > 1; v++){
            newEdges.push_back({compMb[i][v], compMb[i][(v + 1) % compMb[i].size()]});
        }
    }
...
void solve() {
    ...
    int n, m;
    cin >> n >> m;
    graph.resize(n); turnedGraph.resize(n);
    for (int i = 0; i < m; i++){
        int a, b;
        cin >> a >> b;
