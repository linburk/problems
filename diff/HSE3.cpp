...
const i32 bit_c = 100003;

std::bitset<bit_c> operator+(std::bitset<bit_c> const &a, std::bitset<bit_c> const &b) {
  std::bitset<bit_c> res;
  for (i32 i = 0, carry = 0; i < bit_c; ++i) {
    res[i] = a[i] ^ b[i] ^ carry;
    carry = (carry && (a[i] ^ b[i])) || (a[i] && b[i]);
  }
  return res;
}
std::bitset<bit_c> mx(std::bitset<bit_c> a, std::bitset<bit_c> b) {
  if (b[bit_c - 1] != a[bit_c - 1])
    return b[bit_c - 1] ? a : b;
  if (b[bit_c - 1] && a[bit_c - 1]) {
    a = a.flip();
    b = b.flip();
...
struct {
  void solve() {
    ...
    vector<vector<std::bitset<bit_c> > > dp(n + 1, vector<std::bitset<bit_c> >(2));
    dp[0][0][bit_c - 1] = 1;
    for (i32 i = 1; i <= n; ++i) {
      dp[i][1] = mx(dp[i - 1][0], dp[i - 1][1]) + a[i - 1];
      dp[i][0] = mx(dp[i - 1][1] + k, dp[i - 1][0]);
    }
    i64 res = 0;
    std::bitset<bit_c> m = mx(dp[n][0], dp[n][1]);
    for (i32 i = 0; i < bit_c; ++i) {
      if (!m[i]) continue;
      i64 add = pow(2, i, mod);
      if (i == bit_c - 1) {
        res = (res - add + mod) % mod;
