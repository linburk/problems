...
struct st
{
    ...
    {
        tr.resize(2 * a.size());
        tr_min.resize(2 * a.size());
        n = static_cast<i32>(a.size());
        for (i32 i = 0; i < n; ++i)
            tr[i + n] = tr_min[i + n] = a[i];
        for (i32 i = n - 1; i >= 0; --i)
        {
            tr[i] = std::max(tr[i << 1], tr[i << 1 | 1]);
            tr_min[i] = std::min(tr_min[i << 1], tr_min[i << 1 | 1]);
        }
    }
    i32 get(i32 l, i32 r)
    {
        ...
        {
            if (l & 1)
                res = std::max(res, tr[l++]);
            if (r & 1)
                res = std::max(res, tr[--r]);
        }
        return res;
    }
    i32 get_min(i32 l, i32 r)
    {
        l += n, r += n;
        i32 res = 1E9;
        for (; r > l; r >>= 1, l >>= 1)
        {
            if (l & 1)
                res = std::min(res, tr_min[l++]);
            if (r & 1)
                res = std::min(res, tr_min[--r]);
        }
        return res;
    }
};
...
struct
{
    void solve()
    {
        ...
        {
            r = std::min(r, next[i]);
            for (i32 j = r; j >= i + a[i] + 1 && tr.get_min(i, j) == 1;)
            {
                if (tr.get(i, j) == j - i)
                    awful[i].push_back(j - i);
                j = std::min(i + a[j - 1], j - 1);
            }
            std::reverse(all(awful[i]));
        }
        for (i32 i = 0; i < q; ++i)
        {
            i32 l, x;
            cin >> l >> x;
            --l;
            auto it = std::lower_bound(all(awful[l]), x);
            if (it == awful[l].end())
            {
                cout << "-1 0\n";
            }
            ...
            {
                cout << *it << " " << std::distance(it, awful[l].end()) << '\n';
            }
        }
    }
    void precalculate();
