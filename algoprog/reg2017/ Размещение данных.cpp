...
struct {
  ...
  void dfs(i32 v, i32 p, i32 p_edge) {
    ...
    for (auto [to, ind] : gr[v]) {
      if (ind == p_edge) { continue; }
      if (to == p && ind != p_edge) { is_bridge[ind] = is_bridge[p_edge] = -1; continue; }
      if (depth[to]) {
  ...
  void solve() {
    ...
    mpi32 match;
    for (i32 i = 0; i < n; ++i) {
      if (!match.count(comp[i])) match[comp[i]] = static_cast<i32>(match.size());
      comp[i] = match[comp[i]];
    }
    comp_sz = static_cast<i32>(match.size());
    vi32 count(comp_sz);
    cgr.resize(comp_sz);
    for (i32 i = 0; i < n; ++i) ++count[comp[i]];
    const i32 mod = 1e9+7;
    for (i32 i = 0; i < n; ++i) {
