...
struct point {
  ...
  long double length() { return std::hypot(x, y); }

  long double radangle() { return std::atan2(y, x); }
...
struct vec {
  ...
  long double length() { return length(A, B); }

  long double radangle() { return radangle(A, B); }
