...
struct dsu {
  vi32 anc, rank;
  i32 sz;
  dsu(i32 n) { anc.resize(n); rank.resize(n); std::iota(all(anc), 0); sz = n; }
  i32 leader(i32 a) { return anc[a] == a ? a : anc[a] = leader(anc[a]); }
...
struct {
  void solve() {
    ...
    cin >> n;
    i32 const p1 = 67;
    i32 const mod = 998544353;
    v<std::unordered_set<i32> > comp(n);
    for (i32 i = 0; i < n; ++i) {
      i32 k;
      cin >> k;
      for (i32 j = 0; j < k; ++j) {
