...
void solve() {
    ...
    vi32 f_, s_;
    vi32 primes;
    vb prime(1001, 1); prime[0] = prime[1] = 0;
    for (i32 i = 2; i <= 34; ++i) {
        if (!prime[i]) continue;
    ...
    for (i32 i = 0; i <= 1000; ++i) if (prime[i]) primes.push_back(i);
    for (i32 i = 0; i < n; ++i) {
        bool C = 1;
        for (auto j : primes) {
            if (f[j * a[i]] || a[i] % j == 0 && f[a[i] / j]) {
                C = 0;
                break;
            }
        }
        C ? ++f[a[i]] = 1 : ++s[a[i]];
        C ? f_.push_back(i) : s_.push_back(i);
    }
    vi32 color(n);
    for (auto i : f_) color[i] = 1;
    for (auto i : s_) color[i] = 2;
    cout << color << '\n';
}
