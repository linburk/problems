...
struct line {
    ...
    f64 operator*(line const &rhs) {
        return (f64)(rhs.b - b) / (k - rhs.k);
    }
};
...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        auto count = [&] (i64 lmb) {
            ...
            v<f64> dot;
            auto add = [&] (line n) {
                for (; ln.size() && ln.back()(dot.back()) > n(dot.back());)
                    dot.pop_back(), ln.pop_back();
                if (ln.empty()) dot.push_back(-1E12);
                else dot.push_back(ln.back() * n);
                ln.push_back(n);
            };
            auto get = [&] (i64 x) -> pi64 {
                i64 idx = std::lower_bound(all(dot), x) - dot.begin() - 1;
                return {ln[idx](x), ln[idx].idx};
            };
            vpi64 dp(m + 1);
            add({0, 0, 0});
            for (i32 i = 1; i <= m; ++i) {
                auto [dpj, j] = get(a[i - 1]);
                dp[i].first = dpj + a[i - 1] * i - pref[i] + lmb;
                dp[i].second = dp[j].second + 1;
                add({-i, dp[i].first + pref[i], i});
            }
            return dp[m];
        };
        count(0);
...
#endif
#if MULTITEST__
    i32 t; cin >> t; for (i32 i = 0; i < t; i++) { solution solve; solve(i); }
#else
