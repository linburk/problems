...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        vpi32 a(n);
        cin >> a;
        auto count = [&] (vpi32 &c) -> i32 {
            v<std::array<pi32, 2> > dp(n + 1);
            for (i32 i = 0; i < n + 1; ++i) {
                for (i32 j = 0; j < 2; ++j) {
                    dp[i][j] = {1E9, 1E9};
                }
            ...
            for (i32 i = 1; i <= n; ++i) {
                auto [a, b] = c[i - 1];
                if (a > dp[i - 1][0].first) {
                    dp[i][0] = {a, dp[i - 1][0].second};
                }
                if (a > dp[i - 1][1].first) {
                    dp[i][0] = {a, std::min(dp[i][0].second, dp[i - 1][1].second)};
                }
                if (b > dp[i - 1][0].first) {
                    dp[i][1] = {b, dp[i - 1][0].second + 1};
                }
                if (b > dp[i - 1][1].first) {
                    dp[i][1] = {b, std::min(dp[i][1].second, dp[i - 1][1].second)};
                }
            }
            return std::min(dp.back()[0].second, dp.back()[1].second);
        };
        i32 rs = count(a);
        std::reverse(all(a));
        rs = std::min(rs, count(a));
        cout << (rs == 1E9 ? -1 : rs) << "\n";
    }
