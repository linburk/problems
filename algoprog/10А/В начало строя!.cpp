...
public:
    ...
    void v_nachalo_stroya(int soldat_leviy, int soldat_praviy){
        Node *left, *right, *equal;
        split(root, soldat_leviy, left, right);
        split(right, soldat_praviy - soldat_leviy + 1, equal, right);
        root = merge(merge(equal, left), right);
    }
    T get(size_t index){
        Node *left, *right, *equal;
        split(root, index, left, right);
...
void solve() {
    ...
    vector<int> a(n);
    iota(all(a), 1);
    cartesianTree<int> stroy(a);
    for (int i = 0; i < m; i++){
        int l, r;
        cin >> l >> r;
        stroy.v_nachalo_stroya(l - 1, r - 1);
    }
    for (int i = 0; i < n; i++) cout << stroy[i] << ' ';
