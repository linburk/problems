...
struct solution {
    i32 n;
    std::deque<i32> a[3];
    void move(i32 c, i32 fr, i32 to, i32 tmp) {
        if (c == 0) return;
        move(c - 1, fr, tmp, to);
        cout << a[fr].front() << " " << fr + 1 << " " << to + 1 << "\n";
        a[to].push_front(a[fr].front());
        a[fr].pop_front();
        move(c - 1, tmp, to, fr);
    }
    ...
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        for (auto &&i : a[0])
            cin >> i;
        for (; a[1].size() || a[0].size();) {
            for (; a[1].empty() || a[0].size() && a[0].front() < a[1].front(); ) {
                cout << a[0].front() << " " << "1 2\n";
                a[1].push_front(a[0].front());
                a[0].pop_front();
            }
            for (i32 j = 0; j < a[2].size(); ++j) {
                if (a[2][j] > a[1].front()) {
                    move(j, 2, 0, 1);
                    move(1, 1, 2, -1);
                    move(j, 0, 2, 1);
