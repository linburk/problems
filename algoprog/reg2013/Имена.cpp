...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        string x, y;
        cin >> x >> y;
        vvi32 posx(26), posy(26);
        for (i32 i = 0; i < x.size(); ++i)
            posx[x[i] - 'a'].push_back(i);
        for (i32 i = 0; i < y.size(); ++i)
            posy[y[i] - 'a'].push_back(i);
        string ans;
        std::function<void(i32, i32)> find = [&] (i32 i, i32 j) {
            for (i32 k = 25; k >= 0; --k) {
                auto itx = std::lower_bound(all(posx[k]), i);
                auto ity = std::lower_bound(all(posy[k]), j);
                if (itx != posx[k].end() && ity != posy[k].end()) {
                    ans += 'a' + k;
                    find(*itx + 1, *ity + 1);
                    return;
                }
