    ...
    for (i32 i = 0; i <= cycle_cnt; ++i) {
      auto dp2 = [](vpi32 &one) -> i32 {
        ...
        for (i32 j = 1; j <= one.size(); ++j) {
          dp[j].first = dp[j-1].second + std::max(one[j-1].first, one[j-1].second);
          dp[j].second = std::max(dp[j-1].first, dp[j-1].second) + one[j-1].second;
        }
