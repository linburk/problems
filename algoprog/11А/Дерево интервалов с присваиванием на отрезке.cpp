...
private:
    ...
    void push(int v, int ls, int rs){
        if (tree[v].bonus != INT_MIN){
            tree[ls].value = tree[rs].value = tree[v].bonus;
            tree[ls].bonus = tree[rs].bonus = tree[v].bonus;
            tree[v].bonus = INT_MIN;
        }
    }
    ...
    ll get(ll i, ll l, ll r, ll v){
        if (l > i || r < i) return INT_MIN;
        if (l == r && r == i) return tree[v].value;
        int m = (l + r) >> 1, ls = v << 1, rs = v << 1 | 1;
        push(v, ls, rs);
        ll _lg = get(i, l, m, ls);
        ll _rg = get(i, m + 1, r, rs);
        return max(_lg, _rg);
    }
};
