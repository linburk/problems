...
void solve() {
    i32 n, c, k;
    cin >> n >> c >> k;
    i32 c_len = static_cast<i32>(std::to_string(c).size());
    i32 mod = std::pow(10, k);
    string str;
    cin >> str;
    vi64 dp(n+1);
    dp[0] = 1;
    for (i32 i = 0; i < n; ++i) {
        if (str[i] == '0') { dp[i+1] = dp[i]; continue; }
        for (i32 j = 1; j <= c_len; ++j) {
            if (j == c_len && stoi(str.substr(i, j)) > c) continue;
