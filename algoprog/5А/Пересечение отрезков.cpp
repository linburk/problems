...
void solve(){
    point A, B, C, D;
    cin >> A >> B >> C >> D;
    bool check1 = ((vec{A, C} % vec{A, B}) * (vec{A, D} % vec{A, B}) <= 0);
    bool check2 =  ((vec{C, A} % vec(C, D)) * (vec{C, B} % vec{C, D}) <= 0);
    bool check3 = (max(A.x, B.x) >= min(C.x, D.x) and max(C.x, D.x) >= min(A.x, B.x));
    bool check4 =  (max(A.y, B.y) >= min(C.y, D.y) and max(C.y, D.y) >= min(A.y, B.y));
