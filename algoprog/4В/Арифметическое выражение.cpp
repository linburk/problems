...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        vvi32 dp(tkn.size(), vi32(tkn.size() + 1));
        for (i32 i = 0; i < tkn.size(); i += 2) {
            dp[i][i + 1] = tkn[i];
        }
        for (i32 len = 3; len <= tkn.size(); len += 2) {
            for (i32 l = 0; l + len <= tkn.size(); ++l) {
                i32 r = l + len;
                bool add = tkn[l + 1] == -1;
                dp[l][r] = std::max(dp[l][r], add ? tkn[l] + dp[l + 2][r] : tkn[l] * dp[l + 2][r]);
                add = tkn[r - 2] == -1;
                dp[l][r] = std::max(dp[l][r], add ? tkn[r - 1] + dp[l][r - 2] : tkn[r - 1] * dp[l][r - 2]);
            }
        }
        std::function<void(i32, i32)> rec = [&] (i32 l, i32 r) {
            if (l == r - 1) {
                cout << tkn[l];
                return;
            }
            bool add = tkn[l + 1] == -1;
            if (dp[l][r] == (add ? tkn[l] + dp[l + 2][r] : tkn[l] * dp[l + 2][r])) {
                cout << '(' << tkn[l] << (add ? '+' : '*');
                rec(l + 2, r);
                cout << ')';
            } else {
