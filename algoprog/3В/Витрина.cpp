...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        vpi32 dm = { { 0, 1 }, { 0, -1 }, { 1, 0 }, { -1, 0 } };
        vvi32 gr(n, vi32(m, 1E9));
        vvi32 pr(n, vi32(m));
        gr[0][k - 1] = 0;
        std::set<p<i32, pi32> > q { { 0, { 0, k - 1 } } };
        for (; q.size(); ) {
            auto [d, crd] = *q.begin();
            auto [x, y] = crd;
            q.erase(q.begin());
            for (i32 i = 0; i < 4; ++i) {
                auto [dx, dy] = dm[i];
                if (x + dx >= 0 && x + dx < n && y + dy >= k - 1 && y + dy < m) {
                    i32 cost = 0;
                    if (dx) {
                        for (i32 i = 0; i < k; ++i)
                            cost += val[x + dx][y - i];
                    ...
                    if (gr[x][y] + cost < gr[x + dx][y + dy]) {
                        q.erase({ gr[x + dx][y + dy], { x + dx, y + dy } });
                        pr[x + dx][y + dy] = i;
                        gr[x + dx][y + dy] = gr[x][y] + cost;
                        q.insert({ gr[x + dx][y + dy], { x + dx, y + dy } });
                    }
                }
            }
        }
