...
struct solution {
	...
	vi32 res;
	vi32 val;
	umpi32 dfs(i32 v, i32 d, i32 p = 0)
	{
		val[v] = p;
		umpi32 cur;
		cur.max_load_factor(0.28);
		for (auto [to, w] : gr[v]) {
			auto son{ dfs(to, d + 1, p ^ (1 << w)) };
			res[v] = std::max({ res[to], res[v], 1 });
			{
				auto t = son.find(val[v]);
				if (t != son.end()) {
					res[v] = std::max(res[v], t->second - d);
				}
			}
			for (i32 k = 0; k < 26; ++k) {
				{
					auto t = son.find(val[v] ^ (1 << k));
					if (t != son.end()) {
						res[v] = std::max(res[v], t->second - d);
					}
				}
			...
			for (auto [x, l] : son) {
				{
					auto t = cur.find(x);
					if (t != cur.end()) {
						res[v] = std::max(t->second + l - 2 * d, res[v]);
					}
				}
				for (i32 k = 0; k < 26; ++k) {
					{
						auto t = cur.find(x ^ (1 << k));
						if (t != cur.end()) {
							res[v] = std::max(t->second + l - 2 * d, res[v]);
						}
					}
			...
			for (auto [x, l] : son) {
				cur[x] = std::max(cur[x], l);
			}
		}
		cur[val[v]] = std::max(cur[val[v]], d);
		return cur;
	}
	void solve()
	{
