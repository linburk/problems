...
struct solution {
	...
	struct st {
		...
		void build(vpi32 &a)
		{
			...
			st[0] = a;
			for (i32 i = 0; i + 1 < st.size(); ++i) {
				for (i32 j = 0; j + (1 << i) < st[0].size(); ++j) {
		...
		i32 get(i32 l, i32 r)
		{
			...
			return std::min(st[d][l], st[d][r - (1 << d)]).second;
		}
	} rmq;
	...
	i32 inline lca(i32 a, i32 b)
	{
		return rmq.get(std::min(in[a], in[b]), std::max(out[b], out[a]));
	}
	i32 inline dist(i32 a, i32 b)
	{
	...
	void build(i32 v, i32 pr, i32 sz)
	{
		i32 c = -1;
		dfs(v, v, c, sz);
		p[c] = pr;
	...
	vi32 clr;
	v<std::priority_queue<pi32, vpi32, std::greater<pi32> > > val, del;
	void upd(i32 v)
	{
		clr[v] ^= 1;
		for (i32 u = v; u != -1; u = p[u]) {
			i32 d = dist(u, v);
			if (clr[v]) {
				val[u].push({ d, v });
			} else {
	...
	i32 get(i32 v)
	{
		i32 d = 1E9;
		for (i32 u = v; u != -1; u = p[u]) {
			for (; del[u].size() && val[u].top() == del[u].top();)
				val[u].pop(), del[u].pop();
			if (val[u].size()) {
				d = std::min(d, dist(v, val[u].top().second));
			}
		}
		return (d == 1E9 ? -1 : d);
	}
