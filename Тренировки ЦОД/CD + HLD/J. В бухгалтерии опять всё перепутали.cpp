...
private:
	...
	void pull(i32 n, i32 l, i32 r)
	{
		if (tr[n].bon) {
			if ((n << 1 | 1) < tr.size()) {
				tr[n << 1].bon += tr[n].bon;
				tr[n << 1 | 1].bon += tr[n].bon;
			}
			tr[n].val += (r - l) * tr[n].bon;
			tr[n].bon = 0;
		}
	}
	void build(vi32 &a, i32 l, i32 r, i32 n)
	{
		if (l == r - 1) {
			tr[n].val = a[l];
			return;
		...
		build(a, l, m, n << 1);
		build(a, m, r, n << 1 | 1);
		tr[n].val = tr[n << 1].val + tr[n << 1 | 1].val;
	}
	void add(i32 ql, i32 qr, i32 val, i32 l, i32 r, i32 n)
	{
		...
		i32 m = (l + r) >> 1;
		add(ql, qr, val, l, m, n << 1);
		add(ql, qr, val, m, r, n << 1 | 1);
		tr[n].val = tr[n << 1].val + tr[n << 1 | 1].val;
	}
	i64 get(i32 ql, i32 qr, i32 l, i32 r, i32 n)
	{
		...
		i32 m = (l + r) >> 1;
		return get(ql, qr, l, m, n << 1) + get(ql, qr, m, r, n << 1 | 1);
	}
...
public:
	...
	void add(i32 ql, i32 qr, i32 val)
	{
		add(ql, qr, val, 0, sz, 1);
	}
	i64 get(i32 ql, i32 qr)
	{
		return get(ql, qr, 0, sz, 1);
	}
};
...
struct solution {
	...
	void dfs0(i32 v, i32 p)
	{
		...
		std::sort(all(gr[v]), [this](i32 a, i32 b) { return sz[a] > sz[b]; });
	}
	void dfs1(i32 v)
	{
		in[v] = t++;
	...
	void add(i32 v, i32 val)
	{
		tr.add(in[v] + 1, out[v], val);
		for (; v != -1; v = pr[head[v]]) {
			tr.add(in[head[v]], in[v] + 1, val);
		}
	}
	i64 get(i32 v)
	{
		i64 res = tr.get(in[v] + 1, out[v]);
		for (; v != -1; v = pr[head[v]])
			res += tr.get(in[head[v]], in[v] + 1);
		return res;
	}
