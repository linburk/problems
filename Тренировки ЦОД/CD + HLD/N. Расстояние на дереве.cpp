...
struct {
	void solve()
	{
		...
		i64 rs = 0;
		std::function<vi32(i32, i32)> dfs = [&](i32 v, i32 p) -> vi32 {
			vi32 cur;
			for (auto to : gr[v]) {
				if (to == p)
					continue;
				...
				if (son.size() > cur.size())
					std::swap(son, cur);
				for (i32 d = 1; d < k; ++d) {
					if (k - d <= cur.size() && d <= son.size())
						rs +=
							1LL * cur[cur.size() - k + d] * son[son.size() - d];
				}
				for (i32 d = 1; d <= son.size(); ++d) {
					cur[cur.size() - d] += son[son.size() - d];
				}
			}
			if (cur.size() >= k)
				rs += cur[cur.size() - k];
