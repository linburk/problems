...
class staticLCA {
	...
	void dfs(map<pair<int, int>, bool> &path, vector<vector<int> > &gr, int v,
			 int past)
	{
		...
		for (int i = 1; i < 30; i++) {
			...
			funCanTo[v][i] = funCanTo[v][i - 1] &
							 funCanTo[binUps[v][i - 1]][i - 1];
			funCanFrom[v][i] = funCanFrom[v][i - 1] &
							   funCanFrom[binUps[v][i - 1]][i - 1];
		}
		for (auto to : gr[v])
			if (to != past)
				dfs(path, gr, to, v);
	}
	void build(map<pair<int, int>, bool> &path, vector<vector<int> > &gr,
			   int root)
	{
...
public:
	...
	staticLCA(map<pair<int, int>, bool> &path, vector<vector<int> > &gr,
			  int root)
	{
		build(path, gr, root);
	}
