...
class staticLCA {
	...
	void dfs(vector<vector<int> > &gr, int v, int depth)
	{
		...
		depths.push_back(make_pair(depth, depths.size()));
		for (auto to : gr[v]) {
			dfs(gr, to, depth + 1);
			euler_tour.push_back(v);
			depths.push_back(make_pair(depth, depths.size()));
...
public:
	...
	int get(int u, int v)
	{
		return euler_tour[RMQ.get(min(first_enter[u], first_enter[v]),
								  max(first_enter[u], first_enter[v]))
							  .second];
	}
...
void solve()
{
	...
	staticLCA LCA(graph, root);
	cin >> m;
	for (int i = 0; i < m; i++) {
		int a, b;
		cin >> a >> b;
		cout << (LCA.is_ancestor(a, b) ? 1 : 0) << endl;
	}
}
