...
{
	return geometry::length(st, end);
}

template <typename T> long double mvector<T>::radangle()
{
	return geometry::radangle(st, end);
}
...
void solve()
{
	point<ld> O, A, B;
	cin >> O >> A >> B;
	mvector<ld> OA(O, A), OB(O, B);
	ld radius = abs(OA % OB) /
				(OA.length() + OB.length() + mvector<ld>{ A, B }.length());
	ld kp = OB.length() / OA.length();
	OA *= kp;
	OA += OB;
	ld lenAR = radius / sin(abs(radangle(OA, OB)));
	OA *= lenAR / OA.length();
	cout << setprecision(8) << fixed << OA.end.x << " " << OA.end.y;
}
