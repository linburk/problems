...
void solve()
{
	point<ll> A, B, C;
	cin >> A >> B >> C;
	mvector<ll> AB(A, B), BC(B, C), AC(A, C), CB(C, B), CA(C, A), BA(B, A);
	ld ang1 = abs(radangle(AB, AC)), ang2 = abs(radangle(BC, BA)),
	   ang3 = abs(radangle(CA, CB));
	cout << setprecision(7) << fixed
		 << max({ ang1, ang2, ang3 }) * 180 / 4 / atan(1);
}
