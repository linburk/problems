...
struct dsu {
  ...
  dsu() {}
  inline i32 leader(i32 a) {
    return anc[a] == a ? a : anc[a] = leader(anc[a]);
  }
  inline void up(i32 a) {
    val[leader(a)].push_back(1);
  }
  inline void unite(i32 a, i32 b) {
    a = leader(a), b = leader(b);
    if (val[a].size() > val[b].size())
      std::swap(a, b);
    anc[a] = b;
    for (i32 j = 0; j < val[a].size(); ++j)
      val[b][val[b].size() - 1 - j] += val[a][val[a].size() - 1 - j];
    val[a].clear();
    val[a].shrink_to_fit();
  }
  inline i32 get(i32 a, i32 k) {
    if (k >= val[leader(a)].size()) return 0;
    return *(val[leader(a)].end() - k - 1);
  }
};
...
struct {
  void solve()
  {
    ...
    d = dsu(n);
    dfs(0);
    for (; order.size(); order.pop()) {
      i32 v = order.top();
      for (i32 j = 0; j < gr[v].size(); ++j) {
        d.unite(gr[v][j], v);
      }
      d.up(v);
      for (i32 j = 0; j < was[v].size(); ++j) {
        ans[was[v][j].first] = d.get(v, was[v][j].second);
      }
    }
    cout << ans << '\n';
  }
