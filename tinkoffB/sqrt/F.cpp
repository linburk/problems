...
struct {
    void solve()
    {
        ...
        auto check_around = [&count](i64 n, i64 b, i64 s) -> i64 {
            const i32 RND = 32;
            for (i64 i = std::max((i64)2, b - RND); i <= b + RND; ++i) {
                if (count(n, i) == s) {
                    return i;
                }
            }
        ...
        i64 st = n - s + 1;
        i64 res = st;
        for (i64 i = 1; i * i <= st; ++i) {
            res = std::min(res, check_around(n, st / i, s));
            res = std::min(res, check_around(n, i, s));
        }
        cout << res << '\n';
    }
    void precalculate();
