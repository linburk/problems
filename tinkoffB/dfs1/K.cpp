...
struct {
  ...
  void solve() {
    ...
    cgr.resize(csz + 1, vi32(csz + 1));
    clcgr.resize(csz + 1);
    for (i32 v = 0; v < n; ++v) {
      for (auto i : gr[v])
        if (member[v] != member[i])
          cgr[member[v]][member[i]] = 1;
    }
    used.assign(csz + 1, 0);
    for (i32 i = 1; i <= csz; ++i) {
      if (!used[i])
    ...
    for (i32 i = 0; i < n; ++i) {
      ml[member[i]] = i;
      mlist[member[i]].push_back(i);
    }
    vpi32 edges;
    for (auto &l : mlist) {
      if (l.size() > 1)
        for (i32 i = 0; i < l.size(); ++i) {
          edges.push_back({l[i] + 1, l[(i + 1) % l.size()] + 1});
        }
    }
    for (i32 i = 1; i <= csz; ++i) {
      for (i32 j = 1; j <= csz; ++j) {
        if (cgr[i][j])
          edges.push_back({ml[i] + 1, ml[j] + 1});
      }
    }
    cout << edges.size() << '\n' << edges;
  }
