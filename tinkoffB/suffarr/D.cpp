...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        for (int i = 0; i < n; ++i)
            scanf("%c", &s[i]);
        std::iota(all(res), 0);
        std::sort(all(res), [&](i32 lhs, i32 rhs) -> bool {
            for (i32 i = 0; i + lhs < n && i + rhs < n; ++i) {
                if (s[i + rhs] == s[i + lhs])
                    continue;
                return s[i + lhs] < s[i + rhs];
            }
            return lhs > rhs;
        });
        for (int i = 0; i < n; ++i) {
...
#if REDIRECT__
    freopen("/Users/dmitrii/xcode_solution/xcode_solution/input.txt", "r", stdin);
    freopen("/Users/dmitrii/xcode_solution/xcode_solution/output.txt", "w", stdout);
