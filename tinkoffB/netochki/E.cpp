...
struct solution {
    ...
    vb appealing()
    {
        ...
        auto cost = [this](vb& a) {
            i32 rs = 0;
            for (auto [i1, e1, i2, e2, i3, e3] : sat)
                if (a[i1] == e1 || a[i2] == e2 || a[i3] == e3)
                    ++rs;
            return rs;
        };
        i32 cur = cost(a);
        ...
        for (;; t *= q) {
            vb an = a;
            for (i32 i = 0, e = ru32() % (n / 2); i <= e; ++i) {
                i32 j = sat[ru32() % m][ru32() % 3 * 2];
                an[j].flip();
            }
            i32 now = cost(an);
            if (now == m)
                return an;
            if (now > cur || std::exp(std::log(now - cur) / t) < r01(ru64)) {
                std::swap(now, cur);
                std::swap(an, a);
            }
        }
