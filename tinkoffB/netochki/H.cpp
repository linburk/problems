...
struct solution {
    f64 p[3] = { 0.4991, 0.4843, 0.0166 };
    i32 depth = 12;
    f64 prob(i32 c)
    {
        f64 rs = 1.0;
        for (i32 i = 0; i < depth; ++i, c /= 3)
            rs *= p[c % 3];
        return rs;
    }
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        for (i32 i = 0; i < depth; ++i)
            c3 *= 3;
        std::priority_queue<::p<f64, i32>, std::vector<::p<f64, i32>>, std::greater<>> q;
        v<string> encode(c3);
        v<string> from(c3);
        um<string, i32> to;
        for (i32 i = 0; i < c3; ++i) {
        ...
        for (; q.size() > 1;) {
            ...
            q.pop();
            tr.push_back({ v1, v2 });
            q.push({ p1 + p2, tr.size() - 1 });
        };
        std::function<void(i32, string)> dfs = [&](i32 v, string s) {
            if (tr[v].first == -1) {
                encode[v] = s;
                return;
            }
        ...
        if (cmd == "encode") {
            string rs;
            for (i32 i = 0; i < n / depth; ++i) {
                rs += encode[to[s.substr(i * depth, depth)]];
            }
            for (i32 i = n % depth; i > 0; --i) {
                rs += (s[n - i] == 'T' ? "1" : s[n - i] == 'K' ? "01"
                                                               : "00");
            }
            cout << rs << '\n';
        } else {
            string rs;
            i32 i = 0;
            for (i32 c = 0; c < n - n % depth; c += depth) {
                i32 v = tr.size() - 1;
                for (; tr[v].first != -1; ++i) {
                    if (s[i] == '0') {
                        v = tr[v].first;
