...
struct {
  void solve() {
    ...
    cin >> g >> r;
    std::sort(all(d));
    vvi32 dist(m, vi32(g, INF));
    dist[0][0] = 0;
    std::deque<pi32> q{{0, 0}};
    auto upd = [&] (i32 n_d, i32 n_v, i32 n_t, bool back) {
      if (dist[n_v][n_t] > n_d) {
        dist[n_v][n_t] = n_d;
        back ? q.push_back({n_v, n_t}) : q.push_front({n_v, n_t});
      }
    };
    for (; q.size();) {
      auto [v, t] = q.front();
      q.pop_front();
      for (i32 n_v = v - 1; n_v != v + 3; n_v += 2) {
        if (n_v >= 0 && n_v < m) {
          i32 w = abs(d[v] - d[n_v]);
          if (t + w < g) upd(dist[v][t] + w, n_v, t + w, 0);
          if (t + w == g) upd(dist[v][t] + w + r * (n_v != m - 1), n_v, 0, 1);
        }
      }
    }
    i32 res = INF;
    for (i32 i = 0; i < g; ++i) {
      res = std::min(res, dist[m - 1][i]);
    }
    cout << (res == INF ? -1 : res) << '\n';
  }
  void precalculate();
