#include <bits/stdc++.h>
#include <climits>

#define LOCAL__ 1 && __APPLE__
#define REDIRECT__ 1 && __APPLE__
#define PRECALCULATE__ 0
#define MULTITEST__ 0

namespace /* clang-format off */ {
#define all(cont) cont.begin(), cont.end()
using std::vector, std::string, std::pair;
#if REDIRECT__
std::ifstream cin; std::ofstream cout;
#else
using std::cin, std::cout;
#endif
namespace /* aliases */ {
using i8 = std::int8_t; using i16 = std::int16_t; using i32 = std::int32_t; using i64 = std::int64_t; using i128 = __int128;
using u8 = std::uint8_t; using u16 = std::uint16_t; using u32 = std::uint32_t; using u64 = std::uint64_t; using u128 = unsigned __int128; using f64 = long double; using f32 = float;
using pi32 = std::pair<i32, i32>; using vpi32 = std::vector<pi32>; using vvpi32 = std::vector<vpi32>; using pi64 = std::pair<i64, i64>;  using vpi64 = std::vector<pi64>; using vvpi64 = std::vector<vpi64>;
using vi8 = std::vector<i8>; using vvi8 = std::vector<vi8>; using vi32 = std::vector<i32>; using vvi32 = std::vector<vi32>; using vi64 = std::vector<i64>; using vvi64 = std::vector<vi64>;
using vb = std::vector<bool>; using vvb = std::vector<vb>; using vc = std::vector<char>; using vvc = std::vector<vc>;
using si32 = std::set<i32>; using spi32 = std::set<pi32>; using si64 = std::set<i64>; using spi64 = std::set<pi64>;
using mpi32 = std::map<i32, i32>; using umpi32 = std::unordered_map<i32, i32>; using mpi64 = std::map<i64, i64>;  using umpi64 = std::unordered_map<i64, i64>;
}
namespace /* in/out op. overloadings */ {
template <typename T, typename V> std::istream &operator>>(std::istream &is, std::pair<T, V> &p) { is >> p.first >> p.second; return is; }
template <typename T> std::istream &operator>>(std::istream &is, std::vector<T> &cont) { for (auto &i : cont) { is >> i; } return is; }
template <typename T, typename V, char SEP = ' ', char ENDL = '\n'> std::ostream &operator<<(std::ostream &os, std::pair<T, V> const &p) {
#if LOCAL__
  os << "<" << p.first << ", " << p.second << ">"; return os;
#else
  os << p.first << SEP << p.second << ENDL; return os;
#endif
}
template <typename T, char SEP = ' ', char ENDL = '\n'> std::ostream &operator<<(std::ostream &os, const std::vector<T> &cont) {
#if LOCAL__
  os << "[";
  for (auto b = cont.begin(), e = cont.end(); b != e; ++b) os << *b << (b + 1 == e ? "" : ", "); os << "]"; return os;
#else
  for (auto b = cont.begin(), e = cont.end(); b != e; ++b) os << *b << (b + 1 == e ? ENDL : SEP); return os;
#endif
}
}
namespace /* functions */ {
#if __cplusplus >= 202002L
void print(auto &&...args) { ((cout << args), ...); }
void input(auto &&...args) { ((cin >> args), ...); }
#endif
template <typename T> void inline unique(std::vector<T> &cont) { cont.erase(std::unique(cont.begin(), cont.end()), cont.end()); }
std::mt19937_64 ru64(std::chrono::steady_clock().now().time_since_epoch().count());
}
} /* clang-format on */

template <class T>
struct p_ {
  T x, y;
  T abs() const { return (x * x + y * y); }
  operator p_<f64>() { return p_<f64>{static_cast<f64>(x), static_cast<f64>(y)}; }
//  auto operator<=>(p_ const &rhs) const = default;
};
template <class T> std::ostream& operator<<(std::ostream& os, p_<T> const &rhs) {
  os << "(" << rhs.x << ", " << rhs.y << ")";
  return os;
}
template <class T> std::istream& operator>>(std::istream& is, p_<T> &rhs) {
  is >> rhs.x >> rhs.y;
  return is;
}
template <class T> p_<T> operator-(p_<T> const &lhs, p_<T> const &rhs) {
  return {lhs.x - rhs.x, lhs.y - rhs.y};
}
template <class T> p_<T> operator+(p_<T> const &lhs, p_<T> const &rhs) {
  return {lhs.x + rhs.x, lhs.y + rhs.y};
}
template <class T> T operator%(p_<T> const &lhs, p_<T> const &rhs) {
  return lhs.x * rhs.y - lhs.y * rhs.x;
}
template <class T> T operator*(p_<T> const &lhs, p_<T> const &rhs) {
  return lhs.x * rhs.x + lhs.y * rhs.y;
}
template <class T> p_<T> operator*(p_<T> const &lhs, T k) {
  return {lhs.x * k, lhs.y * k};
}
template <class T> bool operator!=(p_<T> const &lhs, p_<T> const &rhs) {
  return !(lhs.x == rhs.x && lhs.y == rhs.y);
}
template <class T> bool operator<(p_<T> const &lhs, p_<T> const &rhs) {
  if (lhs.x == rhs.x) return lhs.y < rhs.y;
  return lhs.x < rhs.x;
}


struct {
  i32 find_bord(vector<p_<i64> > &pnt, i32 k, p_<i64> p) {
    i32 l = 0, r = static_cast<i32>(pnt.size()) + 1;
    bool rotate = std::signbit((pnt[k] - p) % (pnt[(k + 1) % pnt.size()] - p));
    for (; r - l > 1;) {
      i32 m = (l + r) >> 1;
      rotate == std::signbit((pnt[k] - p) % (pnt[(k + m) % pnt.size()] - p)) ? l = m : r = m;
    }
    return (k + l) % pnt.size();
  }
  i32 find_tg(vector<p_<i64> > &pnt, p_<i64> p, i32 l, i32 r) {
    bool rotate = std::signbit((pnt[l] - p) % (pnt[(l + 1) % pnt.size()] - pnt[l]));
    bool rot_null = (pnt[l] - p) % (pnt[(l + 1) % pnt.size()] - pnt[l]) == 0;
    for (; r - l > 1;) {
      i32 m = (l + r) >> 1;
      bool sign = std::signbit((pnt[m] - p) % (pnt[(m + 1) % pnt.size()] - pnt[m]));
      bool null = ((pnt[m] - p) % (pnt[(m + 1) % pnt.size()] - pnt[m])) == 0;
      (rot_null && null) || (!rot_null && rotate == sign && !null) ? l = m : r = m;
    }
    return r % pnt.size();
  }
  pi32 tangent(vector<p_<i64> > &pnt, p_<i64> p) {
    i32 bord = find_bord(pnt, 0, p);
    i32 tg1 = find_tg(pnt, p, 0, bord);
    i32 tg2 = find_tg(pnt, p, bord, static_cast<i32>(pnt.size()));
    return {tg1, tg2};
  }
  bool seg_int(p_<i64> a, p_<i64> b, p_<i64> c, p_<i64> d) {
    bool check1 = std::signbit((c - a) % (b - a)) != std::signbit((d - a) % (b - a)) || (c - a) % (b - a) == 0 || (d - a) % (b - a) == 0;
    bool check2 = std::signbit((a - c) % (d - c)) != std::signbit((b - c) % (d - c)) || (a - c) % (d - c) == 0 || (b - c) % (d - c) == 0;
    bool check3 = (std::max(a.x, b.x) >= std::min(c.x, d.x) && std::max(c.x, d.x) >= std::min(a.x, b.x));
    bool check4 = (std::max(a.y, b.y) >= std::min(c.y, d.y) && std::max(c.y, d.y) >= std::min(a.y, b.y));
    return check1 & check2 & check3 & check4;
  }
  bool is_intersect(vector<p_<i64> > &pnt, p_<i64> &p, i32 k) {
    i32 bord  = find_bord(pnt, k, p);
    std::set<pair<p_<i64>, p_<i64> > > uniq;
    for (i32 i = -20; i < 20; ++i) {
      if (seg_int(
           pnt[k], p,
           pnt[(bord + i + 20 * pnt.size()) % pnt.size()], pnt[(bord + i + 20 * pnt.size() + 1) % pnt.size()])) {
        uniq.insert({pnt[(bord + i + 20 * pnt.size()) % pnt.size()], pnt[(bord + i + 20 * pnt.size() + 1) % pnt.size()]});
      };
    }
    for (i32 i = -20; i < 20; ++i) {
      if (seg_int(
           pnt[k], p,
           pnt[(k + i + 20 * pnt.size()) % pnt.size()], pnt[(k + i + 20 * pnt.size() + 1) % pnt.size()])) {
        uniq.insert({pnt[(k + i + 20 * pnt.size()) % pnt.size()], pnt[(k + i + 20 * pnt.size() + 1) % pnt.size()]});
      }
    }
    return uniq.size() > 2;
  }
  void solve() {
    i32 n, k;
    cin >> n >> k; --k;
    vector<p_<i64> > pnt(n);
    cin >> pnt;
    i32 m;
    cin >> m;
    vector<f64> dist(n, 1E12);
    dist[k] = 0;
    for (i32 i = 1; i < n; ++i)
      dist[(k + i) % n] = std::min(dist[(k + i) % n], std::sqrt((pnt[(k + i) % n] - pnt[(k + i - 1 + n) % n]).abs()) + dist[(k + i - 1 + n) % n]);
    for (i32 i = 1; i < n; ++i)
      dist[(k - i + n) % n] = std::min(dist[(k - i + n) % n], std::sqrt((pnt[(k - i + n) % n] - pnt[(k - i + 1 + n) % n]).abs()) + dist[(k - i + 1 + n) % n]);
    cout << std::setprecision(9) << std::fixed;
    for (i32 i = 0; i < m; ++i) {
      p_<i64> p;
      cin >> p;
      if (!is_intersect(pnt, p, k)) {
        cout << std::sqrt((p - pnt[k]).abs()) << "\n";
      } else {
        auto tg = tangent(pnt, p);
        cout << std::min(std::sqrt((pnt[tg.first] - p).abs()) + dist[tg.first], std::sqrt((pnt[tg.second] - p).abs()) + dist[tg.second]) << "\n";
      }
    }
  }
  void precalculate();
} solution;

i32 main(i32 argc, char *argv[], char *envp[]) /* clang-format off */ {
  cin.tie(nullptr); std::ios_base::sync_with_stdio(false);
  std::setlocale(LC_ALL, "Russian");
#if REDIRECT__
  cin.open(std::getenv("FINPATH"));
  cout.open(std::getenv("FOUTPATH"));
#endif
#if PRECALCULATE__
  solution.precalculate();
#endif
#if MULTITEST__
  i32 t; cin >> t; for (i32 i = 0; i < t; i++)
#endif
  solution.solve();
#if REDIRECT__
  cin.close();
  cout.close();
#endif
  return 0;
} /* clang-format on */

