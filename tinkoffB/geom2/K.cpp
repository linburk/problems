...
struct {
  ...
  void solve() {
    ...
    for (i32 i = 0; i < ch.size(); ++i) {
      i32 l = 0, r = static_cast<i32>(ch.size());
      for (; r - l > 2;) {
        i32 m1 = (l * 2 + r) / 3;
        i32 m2 = (l + 2 * r) / 3;
        p_ p1 = ch[(m1 + i) % ch.size()], p2 = ch[(m2 + i) % ch.size()];
        (p1 - ch[i]).abs() > (p2 - ch[i]).abs() ? l = m1 : r = m2;
      }
      for (i32 j = l - 100; j <= r + 100; ++j) {
        p_ pj = ch[(j + 100 * ch.size()) % ch.size()];
        max_d = std::max(max_d, (pj - ch[i]).abs());
      }
    }
    cout << std::setprecision(20) << std::fixed << std::sqrt(max_d) << '\n';
  }
  void precalculate();
} solution;
