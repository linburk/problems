...
struct solution {
    ...
    i64 cur = 0;
    i32 cl = 0, cr = -1;
    void opp(i32 pos, bool add)
    {
        cur -= 1ll * cnt[a[pos]] * (cnt[a[pos]] - 1) / 2;
        add ? ++cnt[a[pos]] : --cnt[a[pos]];
        cur += 1ll * cnt[a[pos]] * (cnt[a[pos]] - 1) / 2;
    }
    void dvc(i32 l, i32 r, i32 ql, i32 qr, i32 k)
    {
        if (l >= r)
            return;
        i32 opt = ql;
