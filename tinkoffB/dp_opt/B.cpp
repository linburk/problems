...
struct solution {
    ...
    f64 f(i32 i, i32 j)
    {
        return (prefsq[j] - prefsq[i]) - static_cast<f64>(pref[j] - pref[i]) * (pref[j] - pref[i]) / (j - i);
    }
    void dvc(i32 k, i32 l, i32 r, i32 ql, i32 qr)
    {
        if (l >= r)
            return;
        i32 opt = ql;
        i32 m = (l + r) >> 1;
        for (i32 i = ql; i < std::min(m, qr); ++i) {
            f64 cost = dp[k - 1][i] + f(i, m);
            if (cost < dp[k][m]) {
                dp[k][m] = cost;
                opt = i;
            }
    ...
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        pref.resize(n + 1);
        for (i32 i = 0; i < n; ++i) {
            prefsq[i + 1] = prefsq[i] + 1ll * x[i] * x[i];
            pref[i + 1] = pref[i] + x[i];
        }
        dp.resize(k + 1, v<f64>(n + 1, 1E16));
        dp[0][0] = 0;
        for (i32 i = 1; i <= k; ++i) {
            dvc(i, 0, n + 1, 0, n + 1);
        }
        cout << std::setprecision(6) << std::fixed << dp[k][n] << '\n';
    }
