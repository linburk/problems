...
struct hash_str {
private:
  i32 const static p = 61;
  i64 const static mod = 1E9 + 7;
  vi64 pow;
  vi64 hash;
  void to_hash(string &str) {
    hash.resize(str.size() + 1);
    hash[0] = 1;
    for (i32 i = 0; i < str.size(); ++i)
      hash[i + 1] = (hash[i] * p % mod + str[i]) % mod;
  }
public:
  i64 substr(i32 i, i32 len) {
    return (hash[i + len] - hash[i] * pow[len] % mod + mod) % mod;
  }
  hash_str(string &str) {
    pow.resize(str.size() + 1);
    pow[0] = 1;
    for (i32 i = 1; i < str.size() + 1; ++i)
      pow[i] = (pow[i - 1] * p) % mod;
    to_hash(str);
  }
};
...
struct {
  void solve() {
    ...
    hash_str th(t), ph(p);
    vi32 ans;
    for (i32 i = 0; i <= (i32)(p.size()) - (i32)(t.size()); ++i) {
      i32 l = 0, r = static_cast<i32>(t.size()) + 1;
      for (; r - l > 1; ) {
        i32 m = (l + r) >> 1;
        th.substr(0, m) == ph.substr(i, m) ? l = m : r = m;
      }
      if (l == t.size() || th.substr(l + 1, t.size() - l - 1) == ph.substr(i + l + 1, t.size() - l - 1))
        ans.push_back(i + 1);
    }
    cout << ans.size() << '\n' << ans;
  }
