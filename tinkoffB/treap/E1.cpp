...
private:
    ...
    using pndp = std::pair<node*, node*>;
    using tndp = std::tuple<node*, node*, node*>;
    pndp split(i32 key, node* n)
    {
        if (!n)
            return { nullptr, nullptr };
        if (n->key < key) {
            auto [l, r] = split(key, n->r);
            n->r = l;
            return { n, r };
        } else {
            auto [l, r] = split(key, n->l);
            n->l = r;
            return { l, n };
        }
    }
    ...
    tndp find(i32 key)
    {
        auto [l, t] = split(key, root);
        auto [m, r] = split(key + 1, t);
        return { l, m, r };
    }
    i32 down_left(node* n)
    {
        if (n->l)
            return down_left(n->l);
        return n->key;
    }
    i32 down_right(node* n)
    {
        if (n->r)
            return down_right(n->r);
        return n->key;
    }
...
public:
    ...
    i32 prev(i32 key)
    {
        ...
        i32 res = l ? down_right(l) : 2E9;
        root = merge(l, r);
        return res;
    }
    i32 next(i32 key)
    {
        auto [l, r] = split(key + 1, root);
        i32 res = r ? down_left(r) : 2E9;
        root = merge(l, r);
        return res;
    }
};
