...
class st final {
  struct node {
    i32 pref, suff, val, sz;
  };
  vector<node> tr;
  i32 sz;
  void relax(node &n, node const &l, node const &r) {
    n.sz = l.sz + r.sz;
    ...
    if (r.suff == r.sz)
      n.suff += l.suff;
    n.val = std::max({l.val, r.val, l.suff + r.pref});
  }
  void build(vi32 const &a, i32 l, i32 r, i32 n) {
    if (l == r - 1) {
      tr[n] = {a[l] == 0, a[l] == 0, a[l] == 0, 1};
      return;
    }
    build(a, l, (l + r) >> 1, n << 1);
  ...
  void update(i32 val, i32 pos, i32 l, i32 r, i32 n) {
    if (l > pos || r <= pos)
      return;
    if (l == r - 1) {
      tr[n] = {val == 0, val == 0, val == 0, 1};
      return;
    }
    update(val, pos, l, (l + r) >> 1, n << 1);
    update(val, pos, (l + r) >> 1, r, n << 1 | 1);
    relax(tr[n], tr[n << 1], tr[n << 1 | 1]);
  }
...
public:
  ...
  void update(i32 val, i32 pos) { return update(val, pos, 0, sz, 1); }
};
