...
class st {
  ...
  void add(i32 val, i32 ql, i32 qr, i32 l, i32 r, i32 n) {
    if (l >= qr || r <= ql)
      return;
    if (ql <= l && r <= qr) {
      tr[n].val += val;
    ...
    add(val, ql, qr, l, (l + r) >> 1, n << 1);
    add(val, ql, qr, (l + r) >> 1, r, n << 1 | 1);
  }
  i64 get(i32 pos, i32 l, i32 r, i32 n) {
    if (l > pos || r <= pos)
      return 0;
    if (l == pos && l == r - 1) {
      return tr[n].val;
    }
    return get(pos, l, (l + r) >> 1, n << 1) +
           get(pos, (l + r) >> 1, r, n << 1 | 1) + tr[n].val;
  }
...
public:
  ...
  i64 get(i32 pos) { return get(pos, 0, sz, 1); }
  void add(i32 val, i32 ql, i32 qr) { add(val, ql, qr, 0, sz, 1); }
};
