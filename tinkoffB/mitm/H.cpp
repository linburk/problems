...
struct solution {
    auto operator()() -> void // solve
    {
        ...
        auto w_cmp = [](item const& lhs, item const& rhs) {
            if (lhs.w == rhs.w) {
                if (lhs.v == rhs.v) {
                    return lhs.m < rhs.m;
                }
        ...
        std::sort(all(it), w_cmp);
        st tr(it);
        item res { 0, 0, 0 };
        for (i64 m = 0; m < 1 << ((n + 1) / 2); ++m) {
            i64 w = 0;
            i64 v = 0;
            ...
            i64 lp = std::distance(it.begin(), std::lower_bound(all(it), item { l - w, 0, 0 }, w_cmp));
            i64 rp = std::distance(it.begin(), std::upper_bound(all(it), item { r - w, LONG_LONG_MAX, LONG_LONG_MAX }, w_cmp));
            if (lp >= rp)
                continue;
            item get = tr.get(lp, rp);
            res = std::max(res, { get.w + w, get.v + v, get.m | m << (n / 2) });
        }
        if (res.v == 0) {
            cout << "0\n";
            return;
        }
        cout << __builtin_popcountll(res.m) << '\n';
        for (i32 i = 0; i < n; ++i) {
            if (res.m >> i & 1) {
                cout << i + 1 << '\n';
            }
