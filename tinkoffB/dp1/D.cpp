...
struct {
  ...
  void solve() {
    i32 n, k;
    cin >> n >> k;
    vi64 dp1_last(k + 2), dp2_last(k + 2), dp1_new, dp2_new;
    // 1 - xX
    // 2 - Xx
    dp1_last[k + 1] = dp2_last[0] = 1;
    for (i32 i = 1; i <= n; ++i) {
      vi64 pref2(k + 2), suff1(k + 2);
      dp1_new.assign(k + 2, 0);
      dp2_new.assign(k + 2, 0);
      for (i32 j = 0; j < k + 1; ++j) {
        pref2[j + 1] = (pref2[j] + dp2_last[j]) % mod;
      }
      for (i32 j = k + 1; j >= 1; --j) {
        suff1[j - 1] = (suff1[j] + dp1_last[j]) % mod;
      }
      for (i32 j = 1; j <= k; ++j) {
        dp1_new[j] = pref2[j];
        dp2_new[j] = suff1[j];
      }
      dp1_last = std::move(dp1_new);
      dp2_last = std::move(dp2_new);
    }
    i64 res = 0;
    for (i32 j = 1; j <= k; ++j) {
      if (n > 1)
        res = (res + dp1_last[j]) % mod;
      res = (res + dp2_last[j]) % mod;
    }
    cout << res << std::endl;
  }
} solution;
