...
i32 const mod = 998244353;
i32 const sz = 3000100;
i64 powll(i128 x, i128 p) {
  if (p <= 0) return 1;
  i64 t = powll(x, p / 2);
  t = t * t % mod;
...
struct {
  i64 fact[sz];
  i64 invfact[sz];
  i64 inline C(i64 n, i64 k) {
    return fact[n] * invfact[n - k] % mod * invfact[k] % mod;
  }
  i64 inline cat(i64 n) {
    if (n % 2) return 0;
    return C(n, n / 2) * powll(n / 2 + 1, mod - 2) % mod;
  }
  void solve() {
    i64 n, m;
    cin >> n >> m;
    ...
    a = cat(2*n);
    if (m <= n)
      b = (C(n + m, n) - C(n + m, n + 1) + mod) % mod;
    for (i32 i = 0; i < 2 * n; i += 2)
      c = (c + cat(i) * cat(2 * n - i) % mod) % mod;
  ...
  void precalculate() {
    ...
    invfact[sz - 1] = powll(fact[sz - 1], mod - 2);
