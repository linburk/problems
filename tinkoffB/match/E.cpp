...
struct
{
    void solve()
    {
        ...
        for (i32 i = 0; i < m; ++i)
        {
            ...
            i32 a, b, c, d;
            cin >> hh >> t >> mm >> a >> b >> c >> d;
            order[i].first = {hh * 60 + mm, hh * 60 + mm + std::abs(c - a) + std::abs(d - b)};
            order[i].second = {
                {a, b},
                {c, d}
            };
        ...
        for (i32 i = 0; i < m; ++i)
        {
            for (i32 j = 0; j < m; ++j)
            {
                if (i != j)
                {
                    i32 a = order[i].second.second.first;
                    i32 b = order[i].second.second.second;
                    i32 c = order[j].second.first.first;
                    i32 d = order[j].second.first.second;
                    i32 te = order[i].first.second;
                    i32 ts = order[j].first.first;
                    if (std::abs(c - a) + std::abs(d - b) + te < ts)
                    {
                        gr[i].push_back(j);
                    }
                }
