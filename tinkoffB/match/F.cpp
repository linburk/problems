...
struct
{
    void solve()
    {
        ...
        for (i32 i = 0; i < n; ++i)
        {
            for (i32 j = 0; j < m; ++j)
            {
                if ((i + j) % 2 && !fill[i * m + j])
                {
                    ...
                    {
                        if (di > -1 && di < n && dj > -1 && dj < m && !fill[di * m + dj])
                        {
                            gr[i * m + j].push_back(di * m + dj);
                        }
                    }
                }
            }
        ...
        for (i32 i = 0; i < n * m; ++i)
            dfs(i, i);
        vb used1(n * m), used2(n * m);
        vb in_matching(n * m);
        for (i32 i = 0; i < n * m; ++i)
        {
            if (match[i] != -1)
                in_matching[match[i]] = 1;
        }
