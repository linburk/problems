...
struct {
  void solve() {
    ...
    vvi32 dist(1 << 13, vi32(n, INF));
    dist[1][0] = 0;
    std::set<pair<i32, pi32>> q{{0, {1, 0}}};
    for (; q.size();) {
      auto [d, tmp] = *q.begin();
      auto [mask, v] = tmp;
      q.erase(q.begin());
      for (auto [to, w] : gr[v]) {
        ...
        for (i32 i = 0; i < 13; ++i) {
          if (mask & (1 << i)) {
            if (is_thirteen && (n_mask & (1 << (i + w) % 13)))
              can = 0;
            n_mask |= 1 << (i + w) % 13;
          }
        }
        if (can && dist[n_mask][to] > n_d) {
          q.erase({dist[n_mask][to], {n_mask, to}});
          dist[n_mask][to] = n_d;
          q.insert({dist[n_mask][to], {n_mask, to}});
        }
      }
    }
    i32 r = INF;
    for (i32 i = 0; i < (1 << 13); ++i)
      r = std::min(r, dist[i][n - 1]);
    cout << (r == INF ? -1 : r) << '\n';
  }
} solution;
