...
i128 powll(i128 x, i128 p) {
  if (p <= 0) return 1;
  i128 t = powll(x, p / 2);
  t = t * t % mod;
  if (p & 1) return t * x % mod;
  return t;
}
i128 mult(i128 a, i128 b) {
  return a * b % mod;
}
i128 add(i128 a, i128 b) {
  return (a + b + mod) % mod;
}
i128 divi(i128 a, i128 b) {
  return a * powll(b, mod - 2) % mod;
}
...
struct {
  void solve() {
    i128 n;
    cin >> n;
    i128 a = divi(mult(n, add(n, 1)), 2);
    i128 b = divi(mult(a, add(mult(2, n), 1)), 3);
    i128 c = mult(a, a);
    i128 const q = 22;
    i128 qpn = powll(q, n);
    i128 d = divi(add(qpn,  -1), q-1);
    i128 e = divi(add(mult(n, qpn), -d), q-1);
    i128 f = add((mult(2, divi(add(mult(a, qpn), -e), q-1))), -e);
    i128 g = mult(4 * 3, powll(2, n-2));
    if (n == 1) g = 4;
    cout << a << "\n" << b << "\n" << c << "\n" << d << "\n" << e << "\n" << f << "\n" << g << "\n";
  }
  void precalculate();
