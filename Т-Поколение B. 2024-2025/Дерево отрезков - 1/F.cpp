...
class st final {
  ...
  vector<node> tr;
  i32 sz;
  void update(i32 pos, i32 val, i32 l, i32 r, i32 n) {
    if (l > pos || r <= pos)
      return;
    if (l == r - 1) {
      tr[n].val += val;
    ...
    update(pos, val, l, (l + r) >> 1, n << 1);
    update(pos, val, (l + r) >> 1, r, n << 1 | 1);
...
public:
  ...
  void update(i32 pos, i32 val) { return update(pos, val, 0, sz, 1); }
...
void solve() {
  ...
  for (i32 i = 0; i < b.size(); ++i)
    match[b[i]] = i;
  st less(b.size() + 1), greater(b.size() + 1);
  i64 r = 0;
  for (i32 i = 0; i < n; ++i)
    less.update(match[a[i]], 1);
  for (i32 i = 0; i < n; ++i) {
    i32 ind = match[a[i]];
    r += less.get(0, ind) * greater.get(ind + 1, b.size() + 1);
    less.update(ind, -1);
    greater.update(ind, 1);
  }
  cout << r << '\n';
}
