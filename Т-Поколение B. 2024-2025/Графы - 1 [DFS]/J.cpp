...
struct {
  ...
  void dfs(i32 v, i32 p, i32 p_edge) {
    ...
    for (auto [to, e_ind] : gr[v]) {
      if (e_ind == p_edge)
        continue;
      if (e_ind != p_edge && to == p) {
        is_bridge[e_ind] = is_bridge[p_edge] = -1;
        continue;
      }
      if (depth[to]) {
        ...
        if (min_depth[to] > depth[v] && is_bridge[e_ind] != -1) {
          is_bridge[e_ind] = 1;
        }
      }
    }
  ...
  i32 dfs2(i32 v, i8 w) {
    ...
    for (auto [to, ind] : gr[v]) {
      if (used[to])
        continue;
      if (is_bridge[ind] == 1) {
        c.push_back(dfs2(to, 1));
      } else {
        c.push_back(dfs2(to, 0));
      }
    }
    if (c.empty())
      return 0 + w;
