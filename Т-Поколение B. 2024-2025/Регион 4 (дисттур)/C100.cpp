...
struct {
	...
	void solve()
	{
		...
		vector<meal> pr(n);
		for (i32 i = 0; i < n; ++i)
			pr[i].ind = i, cin >> pr[i].t >> pr[i].k;
		std::sort(all(pr), [](meal a, meal b) { return a.t > b.t; });
		vi32 ans;
		std::set<pi64> rem;
		for (i32 i = 0; i < n; ++i) {
			i64 diff_t = pr[i].t - (i < n - 1 ? pr[i + 1].t : 0);
			i64 can_eat = diff_t * c;
			rem.insert({ pr[i].k, pr[i].ind });
			for (; rem.size();) {
				i64 ost = rem.begin()->first;
				i64 i_b = rem.begin()->second;
				rem.erase(rem.begin());
				if (ost - can_eat <= 0) {
					can_eat -= ost;
					ans.push_back(i_b + 1);
