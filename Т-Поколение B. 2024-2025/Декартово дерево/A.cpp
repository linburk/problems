...
private:
    ...
    pnd split(i64 k, nd* n)
    {
        if (!n)
            return { 0, 0 };
        if (n->k < k) {
            auto [l, r] = split(k, n->r);
            n->r = l;
            upd(n);
            return { n, r };
        } else {
            auto [l, r] = split(k, n->l);
            n->l = r;
            upd(n);
            return { l, n };
        }
...
public:
    ...
    {
        auto [l, r] = split(k, rt);
        auto [m, rr] = split(k + 1, r);
        nd* n = m ? m : new nd(k);
        rt = merge(merge(l, n), rr);
    }
    i64 get(i32 l, i32 r)
    {
        auto [ll, rr] = split(l, rt);
        auto [mm, rr2] = split(r, rr);
        i64 rs = val(mm);
        rt = merge(merge(ll, mm), rr2);
        return rs;
    }
};
