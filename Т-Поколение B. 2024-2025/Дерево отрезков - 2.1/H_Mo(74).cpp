...
struct {
  void solve() {
    ...
    const i32 BLOCK_SIZE = 700;
    vector<vector<query>> blocks(order.size() / BLOCK_SIZE + 1);
    for (i32 i = 0; i < q.size(); ++i) {
      blocks[q[i].l / BLOCK_SIZE].push_back(q[i]);
    }
    vi32 count(n + 1);
    i32 all = 0;
    auto add = [&](i32 ind) {
      if (0 == count[order[ind]]++)
        ++all;
    };
    auto del = [&](i32 ind) {
      if (1 == count[order[ind]]--)
        --all;
    };
    for (i32 l = 0, r = 0, i = 0; i < blocks.size(); ++i) {
      if (blocks[i].empty())
        continue;
      std::sort(all(blocks[i]), [](const query &lhs, const query &rhs) {
        return lhs.r < rhs.r;
      });
      for (i32 j = l; j <= r; ++j)
        count[order[j]] = 0;
