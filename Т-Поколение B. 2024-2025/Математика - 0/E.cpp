...
i32 gcd(i32 a, i32 b, i32 &x, i32 &y) {
  ...
  i32 x1, y1;
  i32 d = gcd(b % a, a, x1, y1);
  x = y1 - (b / a) * x1;
  y = x1;
  return d;
}
i64 powmod(i64 x, i64 p, i64 mod) {
  if (!p) return 1;
  i64 t = powmod(x, p / 2, mod);
  t = t * t % mod;
...
struct {
  ...
  void solve() {
    ...
    i64 x0, p;
    if (a % std::gcd(n, m) != b % std::gcd(n, m)) {
      cout << "NO\n";
      return;
    }
    i64 d = std::gcd(n, m);
    i64 t1 = std::gcd(std::gcd(n, b - a), m);
    i64 mt = m / t1, bat = (b - a) / t1, nt = n / t1;
    x0 = (a + (n * ((bat *
          powmod(nt, phi(mt) - 1, mt)
          ) % mt))) % (n * m / d);
    p = n * m / d;
    // x == a (mod n)
    // x = a + yn
  ...
  void precalculate() {
    ...
    for (i32 i = 2; i < sz; ++i) {
      ...
      for (i32 j = 0; j < prs.size(); ++j) {
        if (1ll * prs[j] * i >= sz || prs[j] > pr[i]) break;
        pr[prs[j] * i] = prs[j];
      }
    }
  }
