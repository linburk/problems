...
struct {
  void solve() {
    i32 n;
    cin >> n;
    vector<pair<i64, vector<p_> > > fence(n);
    for (i32 i = 0; i < n; ++i) {
      i32 k;
      cin >> k;
    ...
    std::sort(all(fence), [] (pair<i64, vector<p_> > const &lhs, pair<i64, vector<p_> > const &rhs) {
      return lhs.first > rhs.first;
    });
    auto in_angle = [] (p_ a, p_ o, p_ b, p_ m) -> bool {
      p_ ob = b - o, om = m - o, oa = a - o;
      return
      (std::signbit(oa % om) == std::signbit(oa % ob))
      &&
      (std::signbit(ob % om) == std::signbit(ob % oa));
    };
    auto area = [] (vector<p_> &sh) -> i64 {
      i64 res = sh[sh.size() - 1] % sh[0];
    ...
    auto localize = [&in_angle] (vector<p_> &fence, p_ p) -> bool {
      ...
      for (l = r_b, r = static_cast<i32>(fence.size()) - 1; r - l > 1;) {
        i32 m = (l + r) >> 1;
        in_angle(fence[fence.size() - 1], fence[cent], fence[m], p) ? l = m : r = m;
      }
      r_b = l;
      l_b = r_b + 1;
      return in_angle(fence[l_b], fence[cent], fence[r_b], p) && in_angle(fence[r_b], fence[l_b], fence[cent], p);
    };
    vb count(n);
    i32 q;
    cin >> q;
    for (i32 i = 0; i < q; ++i) {
      ...
      print("{", p.x, ", ", p.y, "}\n");
      for (i32 j = 0; j < n; ++j)
        print(j+1, ": ", localize(fence[j].second, p), "\n");
#endif
      for (; r - l > 1;) {
        i32 m = (l + r) >> 1;
    ...
    for (i32 i = 0; i < n; ++i) {
      ...
      if (count[i] && i - 1 >= 0) res -= area(fence[i - 1].second);
    }
    cout << std::setprecision(6) << std::fixed << res / 2.L << '\n';
  }
  void precalculate();
} solution;
