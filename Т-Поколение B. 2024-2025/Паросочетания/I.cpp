...
struct
{
    void solve()
    {
        ...
        std::sort(all(order), [&](i32 v1, i32 v2) -> bool { return w[v1] > w[v2]; });
        vi32 match(n, -1);
        vi32 used(n, -1);
        ...
        for (i32 i = 0; i < n; ++i)
        {
            ...
            {
                squad.push_back({v + 1});
                for (v = match[v]; v != -1; v = match[v])
                {
                    squad.back().push_back(v + 1);
                }
            }
        }
        cout << sum << ' ' << squad.size() << '\n';
        for (i32 i = 0; i < squad.size(); ++i)
        {
            cout << squad[i].size() << ' ' << squad[i] << '\n';
        }
    }
    void precalculate();
} solution;
