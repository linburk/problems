...
struct solution {
    vi32 appealing(i32 n, i32 k)
    {
        ...
        for (i32 i = 0; i < n; ++i)
            sum[i / nk] += a[i];
        for (i32 i = 0; i < k; ++i)
            cur += std::abs(sum[i] - rb);
        auto recalc = [&](i32 i, i32 j) -> i32 {
            i32 now = cur;
            now -= std::abs(sum[i / nk] - rb);
            sum[i / nk] -= a[i];
            sum[i / nk] += a[j];
            now += std::abs(sum[i / nk] - rb);
            now -= std::abs(sum[j / nk] - rb);
            sum[j / nk] -= a[j];
            sum[j / nk] += a[i];
            now += std::abs(sum[j / nk] - rb);
            std::swap(a[i], a[j]);
        ...
        for (i32 p = 0; p < n * n * k; ++p, t *= q) {
            i32 i = ru64() % n;
            i32 j = ru64() % n;
            i32 now = recalc(i, j);
            if (now == 0)
                return a;
            if (now < cur || std::exp(std::log(cur - now) / t) < r01(ru64)) {
                cur = now;
            } else {
                recalc(i, j);
            }
