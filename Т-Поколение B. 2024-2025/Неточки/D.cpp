...
struct solution {
    i32 n, k;
    v<std::bitset<256>> ctrl0, ctrl1;
    p<i32, vi32> appealing()
    {
        std::uniform_real_distribution<f32> r01(0, 1);
        auto count = [&](vi32& g) {
            std::bitset<256> c0, c1;
            for (auto i : g) {
                c0[i] = 1;
                c1[i] = 1;
                c0 |= ctrl0[i];
            ...
            return std::max(pi32 { c0.count(), 0 }, pi32 { c1.count(), 1 });
        };
        f64 t = 1.0;
        f64 q = 0.999;
        vi32 a(k);
        ...
        for (; cur.first < n; t *= q) {
            ...
            a[i] = j;
            pi32 now = count(a);
            if (now > cur || std::exp((now.first - cur.first) / t) > r01(ru32)) {
                cur = now;
            } else {
                a[i] = pr;
            }
    ...
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        auto rs = appealing();
        std::sort(all(rs.second));
        unique(rs.second);
        for (auto&& i : rs.second)
            ++i;
        cout << rs.first + 1 << "\n"
             << rs.second.size() << "\n"
             << rs.second << "\n";
