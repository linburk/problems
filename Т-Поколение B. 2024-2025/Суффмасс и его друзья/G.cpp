...
using vi16 = std::vector<i16>; using vvi16 = std::vector<vi16>; using vi32 = std::vector<i32>; using vvi32 = std::vector<vi32>; using vi64 = std::vector<i64>; using vvi64 = std::vector<vi64>; using vu64 = std::vector<u64>; using vvu64 = std::vector<vu64>;
...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        i32 st = 1;
        vi32 s;
        auto cmp1 = [&](i32 lhs, i32 rhs) {
            return s[lhs] < s[rhs];
        };
        auto cmp2 = [&](i32 lhs, i32 rhs) {
            if (c[lhs] == c[rhs])
                return c[(lhs + st) % n] < c[(rhs + st) % n];
            return c[lhs] < c[rhs];
        };
        cin >> n >> m;
        s.resize(n);
        ...
        suff.resize(n);
        std::iota(all(suff), 0);
        std::sort(all(suff), cmp1);
        c.resize(n);
        for (i32 i = 0; i < n; ++i)
            c[i] = s[i];
        for (; st < 2 * n; st <<= 1) {
            std::sort(all(suff), cmp2);
            vi32 cn(n);
            for (i32 i = 1; i < n; ++i) {
                i32 l1 = suff[i - 1];
                i32 r1 = (suff[i - 1] + st) % n;
                i32 l2 = suff[i];
                i32 r2 = (suff[i] + st) % n;
                cn[suff[i]] = cn[suff[i - 1]];
                if (c[l1] != c[l2] || c[r1] != c[r2])
                    cn[suff[i]] = cn[suff[i - 1]] + 1;
            }
            c = cn;
        }
        //        for (i32 i = 1;  i < n; ++i) {
        //            for (i32 j = suff[i]; j + 1 < n; ++j) {
        //                cout << s[j];
        //            }
        //            cout << '\n';
        //        }
        ...
        for (i32 i = 0; i + 1 < n; ++i) {
            ...
            i32 j = suff[pos[i] + 1];
            for (; j + p < n && i + p < n && s[i + p] == s[j + p]; ++p)
                ;
            lcp[pos[i]] = p;
        }
        i32 ri = 0;
        pi64 res;
        using ti64 = std::tuple<i64, i64, i64>;
        v<ti64> q;
        for (i32 i = 1; i < n; ++i) {
            i64 c = 1;
            i64 mpr = n - suff[i] - 1;
            if (mpr > res.first * res.second)
                res = { mpr, c }, ri = suff[i];
            for (; q.size(); q.pop_back()) {
                auto [pc, ph, pi] = q.back();
                if (lcp[i] <= ph) {
                    c += pc;
                    if (c * ph > res.first * res.second) {
                        res.first = ph;
                        res.second = c;
                        ri = suff[pi];
                    }
