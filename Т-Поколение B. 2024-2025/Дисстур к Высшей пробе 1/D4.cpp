...
struct solution {
	vvi32 gr;
	i32 n, k;
	vvi32 dp; // 0-n | 0/1 (ret/down)
	void dfs(i32 v, i32 p)
	{
		std::multiset<i32> cur1;
		::p<i32, std::multiset<i32> > cur2;
		cur2.first = -1;
		i32 sum1 = 0;
		i32 sum2 = 0;
		for (auto to : gr[v]) {
			if (to != p) {
				dfs(to, v);
				if (cur1.size() && cur1.size() == k - 1 &&
					sum1 - *cur1.begin() + dp[to][0] + 2 > sum1) {
					sum1 -= *cur1.begin();
					cur1.erase(cur1.begin());
					cur1.insert(dp[to][0] + 2);
				} else if (cur1.size() < k - 1) {
					cur1.insert(dp[to][0] + 2);
					sum1 += dp[to][0] + 2;
				}
				...
				i32 tsum21 = (k != 1 ?
								  sum2 -
									  (cur2.second.size() &&
											   cur2.second.size() == k - 1 ?
										   *cur2.second.begin() :
										   0) +
									  dp[to][0] + 2 :
								  0);
				i32 tsum22 = sum2 -
							 (cur2.first != -1 ? dp[cur2.first][1] + 1 : 0) +
							 dp[to][1] + 1;
				i32 tsum221 =
					tsum22 -
					(cur2.second.size() && cur2.second.size() == k - 1 ?
						 *cur2.second.begin() :
						 0) +
					(cur2.first != -1 ? dp[cur2.first][0] + 2 : 0);
				if (tsum21 == std::max({ tsum22, tsum221, tsum21, sum2 })) {
					if (cur2.second.size() && cur2.second.size() == k - 1) {
						cur2.second.erase(cur2.second.begin());
					}
					cur2.second.insert(dp[to][0] + 2);
					sum2 = tsum21;
				} else if (tsum22 == std::max({ tsum22, tsum221, sum2 })) {
					cur2.first = to;
					sum2 = tsum22;
				} else if (k != 1 && tsum221 == std::max({ tsum221, sum2 })) {
					if (cur2.second.size() && cur2.second.size() == k - 1) {
						cur2.second.erase(cur2.second.begin());
					}
					if (cur2.first != -1) {
						cur2.second.insert(dp[cur2.first][0] + 2);
					}
					cur2.first = to;
					sum2 = tsum221;
				}
