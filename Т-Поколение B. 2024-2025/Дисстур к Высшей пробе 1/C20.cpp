...
struct solution {
	...
	i32 div(i32 l, i32 r, i32 i)
	{
		...
		i32 rs = 1E9;
		for (i32 m = l; m <= r; ++m) {
			i32 tl = div(l, m, i + 1) + pref[m][i] - pref[l][i];
			i32 tr = div(m, r, i + 1) + (r - m) - (pref[r][i] - pref[m][i]);
			rs = std::min(rs, tl + tr);
		}
		return was[l][r][i] = rs;
	}
	void solve()
	{
		...
		was.clear();
		cin >> n >> k;
		pref.resize(n + 1, vi32(k));
		was.resize(n, vvi32(n + 1, vi32(k, -1)));
		for (i32 i = 0; i < n; ++i) {
			string s;
			cin >> s;
			for (i32 j = 0; j < k; ++j) {
				bool bit = (s[j] == '1');
				pref[i + 1][j] = pref[i][j] + bit;
			}
		}
		cout << div(0, n, 0) << '\n';
	}
