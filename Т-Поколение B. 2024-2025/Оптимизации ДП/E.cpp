...
struct solution {
    auto operator()(i32 n_test = 0) -> void // solve
    {
        ...
        for (i32 i = 0; i < m; ++i)
            pref[i + 1] = pref[i] + a[i];
        vvi64 dp(p + 1, vi64(m + 1));
        v<v<line>> ln(p + 1);
        v<v<f64>> dot(p + 1);
        auto add = [&](line n, i32 i) {
            for (; ln[i].size() && ln[i].back()(dot[i].back()) > n(dot[i].back());)
                dot[i].pop_back(), ln[i].pop_back();
            if (ln[i].empty())
                dot[i].push_back(-1E12);
            else
                dot[i].push_back(ln[i].back() * n);
            ln[i].push_back(n);
        };
        auto get = [&](i64 x, i32 i) {
            i64 idx = std::lower_bound(all(dot[i]), x) - dot[i].begin() - 1;
            return ln[i][idx](x);
        };
        add({ 0, 0 }, 0);
        for (i32 i = 0; i < p; ++i) {
            for (i32 j = 1; j <= m; ++j) {
                dp[i + 1][j] = get(a[j - 1], i) + a[j - 1] * j - pref[j];
                add({ -j, dp[i + 1][j] + pref[j] }, i + 1);
            }
        }
        cout << dp[p][m] << '\n';
    }
