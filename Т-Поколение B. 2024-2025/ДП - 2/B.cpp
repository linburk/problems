...
struct {
  void solve() {
    ...
    vector<vvi32> dp(n + 1, vvi32(10, vi32(10)));
    for (i32 i = 100; i < 1000; ++i) {
      dp[3][i / 100][i / 10 % 10] += is_prime[i];
    }
    for (i32 i = 4; i <= n; ++i) {
      for (i32 p1 = 1; p1 < 10; ++p1) {
        for (i32 p2 = 1; p2 < 10; ++p2) {
          for (i32 np = 1; np < 10; ++np) {
            if (is_prime[np * 100 + p2 * 10 + p1]) {
              dp[i][np][p2] = (dp[i][np][p2] + dp[i - 1][p2][p1]) % mod;
            }
          }
        }
      }
