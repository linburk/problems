...
struct {
  void solve() {
    ...
    f64 a = -2 * (o2 - o1).x(), b = -2 * (o2 - o1).y(), c = (o2 - o1).x() * (o2 - o1).x() + (o2 - o1).y() * (o2 - o1).y() + r1 * r1 - r2 * r2;
    i64 rad = r1;
    gpl::vector<i64> O = o1;
    gpl::line<i64> line(a, b, c);
    gpl::vector M(line.norm() & line);
    f64 len_OM = M.abs();
    if (len_OM - eps > rad) {
      cout << "0\n";
    } else if (len_OM + eps > rad && len_OM - eps < rad) {
      cout << "1\n" << (M + O).x() << " " << (M + O).y() << "\n";
    } else {
      cout << "2\n";
      f64 len_ = std::sqrt(rad * rad - M.abs() * M.abs());
      gpl::vector<f64> direct(-line.B(), line.A());
      direct *= len_ / direct.abs();
      gpl::vector M1 = (M + O + direct), M2 = (M + O - direct);
      gpl::vector H = ((M1 + M2) * 0.5);
      cout << H.x() << " " << H.y() << "\n";
      cout << (H - O).abs() << " " << (M1 - H).abs() << "\n";
      cout << M1.x() << " " << M1.y() << "\n";
      cout << M2.x() << " " << M2.y() << "\n";
    }
  }
