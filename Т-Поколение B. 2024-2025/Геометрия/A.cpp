...
struct {
  void solve() {
    pi32 a, b, c;
    cin >> a >> b >> c;
    auto angle = [] (pi64 a, pi64 b, pi64 c) -> f64 {
      a.first -= b.first;
      a.second -= b.second;
      c.first -= b.first;
      c.second -= b.second;
      return std::atan2(std::abs(a.first * c.second - a.second * c.first), a.first * c.first + a.second * c.second);
    };
    cout << std::setprecision(7) << std::fixed << 180 / atan(1) / 4 * std::max({angle(a, b, c), angle(a, c, b), angle(b, a, c)}) << '\n';
  }
  void precalculate();
} solution;
