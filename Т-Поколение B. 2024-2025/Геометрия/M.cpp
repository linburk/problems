...
bool on_ray(gpl::vector<f64> a, gpl::vector<f64> b, gpl::vector<f64> p) {
  return (b - a) * (p - a) > 0;
}

bool on_seg(gpl::segment<f64> a, gpl::vector<f64> p) {
  return on_ray(a.end(), a.st(), p) && on_ray(a.st(), a.end(), p);
}

template <typename T>
f64 dist(gpl::vector<T> a, gpl::vector<T> c) { // point a point b
  return (c - a).abs();
}
template <typename T>
f64 dist(gpl::line<T> a, gpl::vector<T> c) { // line a point c
  return ((a.norm() & c & a) - c).abs();
}
f64 dist(gpl::vector<i64> a, gpl::vector<i64> b, gpl::vector<i64> c) { // ray a->b point c
  if (on_ray(a, b, c))
    return dist(gpl::line<i64>(a, b), c);
  return (c - a).abs();
}

f64 dist(gpl::segment<i64> a, gpl::vector<i64> c) { // seg a point b
  if (on_seg(a, c))
    return dist(gpl::line<i64>(a), c);
  return std::min((a.end() - c).abs(), (a.st() - c).abs());
}

f64 dist(gpl::line<i64> a, gpl::line<i64> c) { // line a line c
  if (std::isnan((a & c).x())) return dist(static_cast<gpl::line<f64>>(a.norm()) & c, static_cast<gpl::line<f64>>(c.norm()) & a);
  else return 0;
}

f64 dist(gpl::segment<i64> a, gpl::segment<i64> c) { // seg a seg b
  f64 d1 = std::min
  ({dist(a, c.st()),
    dist(a, c.end()),
    dist(c, a.st()),
  ...
  gpl::vector<f64> m((gpl::line<i64>(a) & (gpl::line<i64>(c))));
  if (!std::isnan(m.x()) && on_seg(a, m) && on_seg(c, m)) d1 = 0;
  return d1;
}

f64 dist(gpl::line<i64> a, gpl::vector<i64> c, gpl::vector<i64> d) { // line a ray c->d
  gpl::line<i64> cd(c, d);
  gpl::vector m(a & cd);
  if (on_ray(c, d, m))
    return 0;
...
f64 dist(gpl::line<i64> a, gpl::segment<i64> c) { // line a seg c
  gpl::vector<f64> m = a & gpl::line<i64>(c);
  if (std::isnan(m.x())) return dist(a, c.st());
  if (on_seg(c, m)) {
    return 0;
  }
  return std::min(dist(a, c.st()), dist(a, c.end()));
}

f64 dist(gpl::segment<i64> a, gpl::vector<i64> c, gpl::vector<i64> d) { // seg a ray c->d
  gpl::line<i64> ab(a), cd(c, d);
  gpl::vector m(ab & cd);
  if (!std::isnan(m.x()) && on_seg(a, m) && on_ray(c, d, m)) return 0;
  return std::min({dist(c, d, a.st()), dist(c, d, a.end()), dist(a, c)});
}

f64 dist(gpl::vector<i64> a, gpl::vector<i64> b, gpl::vector<i64> c, gpl::vector<i64> d) { // ray a->b ray c->d
  gpl::line<i64> ab(a, b), cd(c, d);
  gpl::vector m(ab & cd);
  if (on_ray(a, b, m) && on_ray(c, d, m))
    return 0;
  return std::min(dist(a, b, c), dist(c, d, a));
}
...
struct {
  void solve() {
    ...
    cout << dist(a, c) << "\n";
    cout << dist(gpl::segment<i64>(c, d), a) << "\n";
    cout << dist(c, d, a) << "\n";
    cout << dist(gpl::line(c, d), a) << "\n";
    
    cout << dist(gpl::segment<i64>(a, b), c) << "\n";
    cout << dist(gpl::segment<i64>(a, b), gpl::segment<i64>(c, d)) << "\n";
    cout << dist(gpl::segment<i64>(a, b), c, d) << "\n";
    cout << dist(gpl::line<i64>(c, d), gpl::segment<i64>(a, b)) << "\n";
    
    cout << dist(a, b, c) << "\n";
    cout << dist(gpl::segment<i64>(c, d), a, b) << "\n";
    cout << dist(a, b, c, d) << "\n";
    cout << dist(gpl::line<i64>(c, d), a, b) << "\n";
    
    cout << dist(gpl::line<i64>(a, b), c) << "\n";
    cout << dist(gpl::line<i64>(a, b), gpl::segment<i64>(c, d)) << "\n";
    cout << dist(gpl::line<i64>(a, b), c, d) << "\n";
    cout << dist(gpl::line<i64>(a, b), gpl::line<i64>(c, d)) << "\n";
