...
struct {
  void solve() {
    ...
    if (std::abs(rad - (o - k).abs()) < eps) {
      cout << "1\n";
      cout << k.x() << " " << k.y() << "\n";
      return;
    }
    gpl::segment<f64> KO(k, o);
    f64 ang = std::asin(rad / KO.abs());
    f64 n_len = std::sqrt(KO.abs() * KO.abs() - rad * rad) / KO.abs();
    gpl::vector m1((KO.rotate(ang) * n_len).end() ), m2((KO.rotate(-ang) * n_len).end());
    cout << "2\n";
    cout << m1.x() << " " << m1.y() << "\n";
    cout << m2.x() << " " << m2.y() << "\n";
  }
