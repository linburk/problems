...
struct vctr {
  i64 x, y;
  vctr() {}
  vctr(i64 x_, i64 y_) { x = x_, y = y_; }
  vctr(vctr const &v) { *this = v; }
  friend std::istream& operator>>(std::istream &is, vctr &n) {
    is >> n.x >> n.y; return is;
  }
  f64 abs() { return std::hypotl(x, y); }
};
vctr operator-(vctr const &lhs, vctr const &rhs) {
  return vctr(lhs.x - rhs.x, lhs.y - rhs.y);
}
i64 operator%(vctr const &lhs, vctr const &rhs) {
...
struct {
  void solve() {
    ...
    i64 len;
    cin >> a >> b >> c >> len;
    f64 max_r = std::max({(a - c).abs() - len, (b - c).abs() - len, 0.l});
    f64 min_r;
    if ((a - b) * (c - b) > 0 && (b - a) * (c - a) > 0) {
      min_r = std::max(0.l, std::abs((b - c) % (b - a) / (b - a).abs()) - len);
    } else {
      min_r = std::max(0.l, std::min((a - c).abs(), (b - c).abs()) - len);
    }
    cout << std::setprecision(7) << std::fixed << min_r << '\n' << max_r << '\n';
  }
  void precalculate();
} solution;
